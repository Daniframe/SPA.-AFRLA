...
for task in TASKS:
    ...
    path = os.path.join("Experiments Data", task, "bayreg")
    ...
    for error in ERRORS:
        ...
        # Train model
        bayreg = BayesianRidge(max_iter = 1000)
        bayreg.fit(X_train, y_train)

        # Feature importance
        feature_importance_df.index = bayreg.feature_names_in_
        feature_importance_df[f"{error}_importance"] = bayreg.coef_

        # Predictions
        y_train_pred = bayreg.predict(X_train)
        y_test_pred = bayreg.predict(X_test)
    ...
    # Save extra params
    if error in {"Lpls", "Lpl"}:
        with open(os.path.join("Experiments Data", task, "bayreg", f"extra_params.json"), "w") as f:
